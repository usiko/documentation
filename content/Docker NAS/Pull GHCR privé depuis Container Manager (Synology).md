#docker #synology #ghcr #devops #nas

Notes de référence, indépendantes de tout projet, sur le pull d'images Docker
**privées** hébergées sur GitHub Container Registry (GHCR) depuis un NAS
Synology (DSM, Container Manager). Découvertes en déployant [[MongoManager]],
mais tout ce qui suit s'applique à n'importe quel projet déployé de la même
façon sur ce NAS.

But du document : garder une trace du cheminement (y compris les impasses)
pour ne pas re-découvrir les mêmes pièges au prochain déploiement Docker sur
ce NAS.
...
Les packages GHCR publiés via `GITHUB_TOKEN` dans une GitHub Action sont
**privés par défaut**. Container Manager doit donc s'authentifier pour
pull une image `ghcr.io/<owner>/<image>:latest`.

### 1.1 Piste évidente qui ne marchait pas : Registre Container Manager
...
- Le workflow Docker du repo concerné avait bien tourné avec succès, image bien publiée (confirmé dans les logs du job).
- Test direct depuis un terminal (hors NAS) avec le vrai flux d'auth Docker Registry v2 (échange de bearer token en 2 étapes, **pas** un simple Basic Auth) → `200 OK`. Donc **le token et l'accès au package fonctionnent** — le problème est spécifique à Synology.
...
### 1.2 Tentative : proxy pull-through local (`ghcr-proxy`)
...
Puis dans le compose du projet concerné : `image: 192.168.1.19:9050/usiko/<image>:latest` (au lieu de `ghcr.io/usiko/...`).
...
### 1.3 Tentative : HTTPS avec certificat auto-signé
...
> `insecure-registries` (cf. §1.5) fait sauter la vérification de certificat de toute façon (HTTPS non fiable **ou** HTTP simple, les deux passent) — donc pas besoin d'aller chercher un vrai certificat signé pour un usage strictement local/LAN. Le HTTPS auto-signé a été une fausse piste, mais les fichiers `cert.pem`/`privkey.pem` peuvent rester en place sans gêner (dans un dossier partagé classique, pas dans les répertoires système gérés par le paquet Container Manager — donc pas de risque qu'ils soient écrasés à une mise à jour).

### 1.4 Piste abandonnée : reverse proxy DSM + domaine public

Idée un temps envisagée : utiliser un domaine déjà configuré (`syn.quentinondet.fr`) + le proxy inversé DSM + Let's Encrypt pour avoir un vrai certificat de confiance sur le proxy. **Rejetée volontairement** : ça exposerait le proxy (et indirectement le token GHCR) sur Internet, alors que le besoin est purement local (LAN). Toujours vérifier ce genre d'implication avant de router quoi que ce soit via un domaine public.

### 1.5 Solution finale : `insecure-registries` au niveau du vrai fichier de config du démon
...
# cf. script setup-insecure-registry.sh en annexe

sudo synosystemctl restart pkg-ContainerManager-dockerd
```

Résultat : le pull passe enfin, aussi bien pour `ghcr-proxy` (192.168.1.19:9050) que pour les images derrière.
...
## 2. Fichiers de référence (chemins clés Synology, Container Manager)
...
À rejouer via une tâche du Planificateur DSM déclenchée au **démarrage** (Boot-up), en filet de sécurité si `dockerd.json` est régénéré au reboot. (Le chemin `ENV_FILE` ci-dessus est celui utilisé pour [[MongoManager]] — à adapter par projet, ou à généraliser si plusieurs projets partagent ce proxy.)
